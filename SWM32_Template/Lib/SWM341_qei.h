...
typedef struct {
    ...
    uint8_t  intMATCHEn;			//POSCNT递增到与MAXCNT相等，或 POSCNT 从MAXCNT递减到 0 中断使能
...
void QEI_IntEn(QEI_TypeDef * QEIx, uint32_t it);		//QEI 中断使能
void QEI_IntDis(QEI_TypeDef * QEIx, uint32_t it);		//QEI 中断关闭
void QEI_IntClr(QEI_TypeDef * QEIx, uint32_t it);		//QEI 中断标志清除
uint32_t QEI_IntStat(QEI_TypeDef * QEIx, uint32_t it);	//QEI 中断状态查询
