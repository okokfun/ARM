/**
  ...
  * 版本:    V1.8.0
  * 日期:    04-November-2016
  * 简介:    此文件提供固件功能，用于管理 EXTI 外设设备的以下功能:
  *           + 初始化和配置
  *           + 中断和标志管理
  *
@verbatim
 ...
 [..] 外部中断/事件行映射如下:
   ...
   (#) EXTI 线路 17 连接至 RTC 报警事件
   (#) EXTI 线路 18 连接到 USB OTG FS 唤醒暂停事件
   (#) EXTI 线路 19 连接到以太网唤醒事件
   (#) EXTI 线路 20 连接到 USB OTG HS(在 FS 中配置)唤醒事件
   (#) EXTI 线路 21 连接到 RTC 篡改和时间戳事件
   (#) EXTI 线路 22 连接至 RTC 唤醒事件
   (#) EXTI 线路 23 连接到 LPTIM 唤醒事件

                       ##### 如何使用这个驱动程序 #####
 ===============================================================================

 [..] 为了将 I/O 引脚用作外部中断源，请执行以下步骤:
   (#) 使用 GPIO_Init() 在输入模式下配置I/O
   (#) 使用 SYSCFG_EXTILineConfig() 为EXTI 线选择输入源引脚
