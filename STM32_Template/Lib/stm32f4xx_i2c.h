...
/**
  ...
  *     在大多数情况下，使用 EV8就足够了。
  *     使用 EV8_2会导致较慢的通信，但确保更可靠的测试。
  *     EV8_2也比EV8更适合在最后一次数据传输上进行测试(在生成停止条件之前)。
  *
  *  @note 如果用户软件不能保证在当前字节传输结束之前管理此事件EV7，
  *        则用户可以同时检查EV7 和BTF 标志(即(I2C_event_MASTER_byte_RECEIVED|I2C_flag_BTF))。
  *		   在这种情况下，通信可能会更慢。
  *
  */
...
/**
  ...
  * 则用户可以同时检查EV2 和BTF 标志(即(I2C_EVENT_SLAVE_BYTE_RECEIVED | I2C_FLAG_BTF))。在这种情况下，通信可能会更慢。
  *
  */
...
void I2C_StructInit(I2C_InitTypeDef* I2C_InitStruct); // 用每个 I2C_InitStruct 成员的默认值填充其默认值。
