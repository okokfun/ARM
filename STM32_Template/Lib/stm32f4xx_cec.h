...
void CEC_StructInit(CEC_InitTypeDef* CEC_InitStruct); // 用默认值填充每个 CEC_InitStruct 成员。
void CEC_Cmd(FunctionalState NewState); // 启用或禁用 CEC 外设设备。
void CEC_ListenModeCmd(FunctionalState NewState); // 启用或禁用 CEC侦听模式。
void CEC_OwnAddressConfig(uint8_t CEC_OwnAddress); // 定义CEC 设备的自有地址。
void CEC_OwnAddressClear(void); // 清除 CEC 设备的自有地址。
...
ITStatus CEC_GetITStatus(uint16_t CEC_IT); // 检查指定的 CEC 中断是否已发生。
